...
import { getMatches, updateMatch } from "../../Services/Match";
...
export const EditMatch: FC<{ matchId: string }> = ({ matchId }) => {
  ...
  const [isFetched, setIsFetching] = useState(true);
  ...
  useEffect(() => {
    ...
    getMatches().then((matches) => {
      ...
      setHomeTeamId(match?.homeTeam || "");
      setAwayTeamId(match?.awayTeam || "");
      setTime(match?.time || new Date());
      setStadiumId(match?.stadium || "");
      setRefereeName(match?.referee || "");
      setLinesMen(match?.linesmen || []);
      setTicketPrice(match?.ticketPrice || 0);
      setIsFetching(false);
    });
  }, [matchId]);
  ...
  function handleTicketPriceChange(
